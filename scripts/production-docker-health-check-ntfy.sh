...
notify_failure() {
    ...
    message="LuxQuote Docker health check failed at $(timestamp) on $(hostname).
    ...
    output="$(
        {
            ...
            for service in $EXPECTED_SERVICES; do
                if ! printf '%s\n' "$running_services" | grep --fixed-strings --line-regexp --quiet "$service"; then
                    printf 'Missing or stopped Compose service: %s\n' "$service"
                    exit 1
                fi
            done
