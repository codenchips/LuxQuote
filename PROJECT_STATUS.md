...
- **Tender-aware Document Packs**: generating a saved pack containing a Quote opens the Tender selector when the project has Tenders. Each selection queues a separate complete combined pack with the matching cover sheet; selecting none produces one cover-free pack, and multiple results can be downloaded together as a Document Pack ZIP.
- **Complete Output History**: successful Document Pack generations now appear with Quote and Schedule outputs. Rows show the saved pack name, revision, Tender, aggregate datasheet choice, and a permission-aware regeneration action; retained legacy pack logs are also included without inventing metadata that was never recorded.
...
- **ntfy login alert wrapper added**: `scripts/production-login-health-check-ntfy.sh` requests `https://quote.tamlite.co.uk/login`, verifies the response contains `LuxQuote`, and posts failures to `https://ntfy.sh/LuxQuoteLogin`. Recommended cadence is every 10 minutes. It now retries by default before alerting to reduce transient proxy/network false positives.
- **Additional ntfy focused alert wrappers added**: `scripts/production-disk-health-check-ntfy.sh`, `scripts/production-docker-health-check-ntfy.sh`, `scripts/production-database-health-check-ntfy.sh`, and `scripts/production-salesforce-health-check-ntfy.sh` post failures to `LuxQuoteDisk`, `LuxQuoteDocker`, `LuxQuoteDatabase`, and `LuxQuoteSalesforce` respectively. `DEPLOYMENT.md` records the crontab lines. The Docker and Database wrappers retry before alerting.
...
- Uploaded PDFs show a first-page thumbnail in the authenticated in-page lightbox. Unsaved uploads/replacements use a local browser preview and preserve the selected filename until the pack is saved.
...
- Quote and Schedule cards have revision-aware options for selected areas and embedded datasheets. Cards show the resolved area names and current datasheet count; template/legacy items that have not been configured, or selections that no longer match a chosen revision, display a refresh action and cannot be queued until corrected.
- Packs containing a Quote reuse the Quick Output Tender selector. A separate full pack is generated for each checked Tender with its matching cover sheet; clearing all Tenders generates one pack without a cover sheet. Tender/project ownership and all underlying output permissions are enforced server-side.
