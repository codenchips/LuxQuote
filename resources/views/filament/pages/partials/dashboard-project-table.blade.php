<div class="overflow-hidden">
    <table class="lux-dashboard-table w-full table-fixed text-sm">
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    <td class="min-w-0 px-3 py-3">
                        <a href="{{ $row['url'] }}" class="block truncate font-medium text-gray-900 hover:underline dark:text-white" title="{{ $row['name'] }}">
                            {{ $row['name'] }}
                        </a>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="4" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ $empty }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
